...
%% /* The grammar follows. */
// CompUnit: Decl CompUnit		{ $$ = root==NULL ? CreatRoot() : $1; $$->next = $2; }
//         | FuncDef CompUnit	{ $$ = root==NULL ? CreatRoot() : $1; $$->next = $2; }
//         | Decl				{ $$ = root==NULL ? CreatRoot() : $1; }
//         | FuncDef			{ $$ = root==NULL ? CreatRoot() : $1; }
// 		;
Start: CompUnits YYEOF			{ $$ = $1; AstRoot = $$; }
	;

CompUnits: CompUnit CompUnits	{ $$ = $1; $$->next = $2; }
		| CompUnit				{ $$ = $1; }
		;
...
FuncDef: INT ID LPAR RPAR Block					{ $$ = newBasicNode("FUNCTION_DECL", $2, -1, NULL, $5, NULL); }
...
Stmt: LVal ASSIGN Exp SEMICOLON				{ $$ = newBasicNode("BINARY_OPERATOR", "=", $3->ivalue, $1, $3, NULL); }	
    | Exp SEMICOLON							{ $$ = $1; }
    | Block									{ $$ = $1; }
...
UnaryExp: PrimaryExp			{ $$ = $1; }
    ...
    | ADD UnaryExp				{ $$ = newBasicNode("UNARY_EXPR", "+", $2->ivalue, NULL, NULL, $2); }
    | SUB UnaryExp				{ $$ = newBasicNode("UNARY_EXPR", "-", -1*$2->ivalue, NULL, NULL, $2); }
...
MulExp: UnaryExp				{ $$ = $1;}
    | MulExp MUL UnaryExp		{ $$ = newBinaryExp("*", $1, $3); $$->ivalue = $1->ivalue * $3->ivalue;}
    | MulExp DIV UnaryExp		{ $$ = newBinaryExp("/", $1, $3); $$->ivalue = $1->ivalue / $3->ivalue;}
    | MulExp MODULO UnaryExp	{ $$ = newBinaryExp("%", $1, $3); $$->ivalue = $1->ivalue % $3->ivalue;}
	;

AddExp: MulExp
    | AddExp ADD MulExp			{ $$ = newBinaryExp("+", $1, $3); $$->ivalue = $1->ivalue + $3->ivalue; }
    | AddExp SUB MulExp			{ $$ = newBinaryExp("-", $1, $3); $$->ivalue = $1->ivalue - $3->ivalue; }
	;
